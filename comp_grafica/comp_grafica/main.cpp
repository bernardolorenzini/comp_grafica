...
bool rotateX=false, rotateY=false, rotateZ=false, moveA = false, moveD = false, moveW = false, moveS = false;
bool aumentar = false, diminuir = false;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            if(triOffset <= -1.0f) triOffset = -1.0f;
            else triOffset -= 0.05f;
            model = glm::translate(model, glm::vec3(triOffset, 0.0f, 0.0f ));
       ...
       /* else if (aumentar)
        {
            ...
            model = glm::translate(2, glm::vec3(0.0f, triOffset, 0.0f ));
        }
        */
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    {
        std::cout << "A key is being pressed" <<endl;
        rotateX = false;
        rotateY = false;
        rotateZ = false;
        moveA = true;
